"use strict";
(function($, ns) {
	ns.MovableCoord = function(pos, options) {
		this._randomKey = "__MOVABLECOORD_" + Math.round(Math.random() * new Date().getTime());
		this._pos = pos || [0,0];
		this._animating = null;
		this._raf = null;
		this._grapOutside = false;
		this._movingPos = [];
		this._subOptions = {};
	...
	ns.MovableCoord.prototype = {
		bind : function(el, options) {
			...
			// create Hammer
			var self = this,
				hammer = new Hammer.Manager($el.get(0), {
					// inputClass :  Hammer.SingleTouchInput
				});
			hammer.add( new Hammer.Pan({
				direction: subOptions.direction,//Hammer.DIRECTION_ALL,
				threshold: 0
			}));
		...
		_panstart : function() {
			...
			this._grab();
			console.info("hold", pos);
			this._grapOutside = this._isOutside(pos, this.options.min, this.options.max);
		...
		_panmove : function(e) {
			...
			//@todo fire "change"
			// console.info("change",pos);
			$("#dot").css("left", pos[0]).css("top", pos[1]);
		},
		...
		_panend : function(e) {
			...
			this._move(offset, true, Infinity);
		...
		_move : function(pos, bBy, maximumDuration) {
			// var self = this;
			// (relPos, callback, isBounce, maximumDuration)
			this[bBy ? "_animateBy" : "_animateTo"](pos, function() {
				...
				this._animateTo( [ Math.min(max[0], Math.max(min[0], pos[0])), Math.min(max[1], Math.max(min[1], pos[1])) ] , function() {
					// @todo call "animationEnd"
					console.info("animationEnd");
				}, true, maximumDuration);
		...
		_afterReleaseProcess : function(param, callback, isBounce, maximumDuration) {
			...
			// @todo call "animation" event
			console.info("animation", param);

			// bCircular 가 true 인데 이벤트 stop 했으면
			// if (isCircular && !retFire) {
		...
		_frame : function(animating) {
			...
			// @todo call "change" 이벤트 발생
			console.info("change",pos, per);
			$("#dot").css("left", pos[0]).css("top", pos[1]);
			return per;
		}
	};
