(function($) {
	var _ua = navigator.userAgent,
		...
		_check = function(method) {
			var v = null;
			if(_patch[method] != undefined) {
				if(typeof _patch[method] == "function") {
					v = _patch[method]();
				} else {
					v = Number(_patch[method]);
				}
				switch (v){
					case -1 : return false;
					case 1 : return true;
		...
		_agent = (function() {
			...
			var browserMatch = /(Chrome|CriOS)[ \/]([\w.]+)/.exec(_ua) ||
				[];
	...
	$.extend({
		...
		useCss3d : function() {
			...
			if(/ios/.test(_agent.os)) {
				...
				if(_agent.osVersion >= "4.1.0") {
					// android 4.1+ blacklist
					if(!/EK-GN120|SM-G386F/.test(ua)) {	// EK-GN120 : Galaxy Camera, SM-G386F : Galaxy Core LTE
						result = true;
					}
				} else {
					if(_agent.osVersion >= "4.0.3" &&
						/SHW-|SHV-|GT-|SCH-|SGH-|SPH-|LG-F160|LG-F100|LG-F180|LG-F200|EK-|IM-A|LG-F240|LG-F260/.test(ua) &&
						!/SHW-M420|SHW-M200|GT-S7562/.test(ua)) {    // SHW-M420 : Galaxy Nexus , SHW-M200 : NexusS , GT-S7562 : Galaxy S duos
						result = true;
					}
				}
